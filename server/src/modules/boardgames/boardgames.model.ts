...
const cachedBoardgames = new Cached(async () => {
  ...
  const boardgamesForSearch = boardgames.map((boardgame) => ({
    boardgame,
    preparedTitle: fuzzysort.prepare(boardgame.title),
  }))

  return {
    list: boardgames,
    byId: boardgamesById,
    listForSearch: boardgamesForSearch,
  }
})
...
export const searchByTitle = async (title: string) => {
  ...
  const result = fuzzysort.go(title, boardgames.listForSearch, {
    key: 'preparedTitle',
    limit: 50,
    threshold: -10000,
  })
  return result.map((r) => r.obj.boardgame)
}
